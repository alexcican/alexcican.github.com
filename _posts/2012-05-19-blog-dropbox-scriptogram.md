...
---
A few months ago I cancelled my hosting accounts and moved all my [websites to Dropbox and GitHub](https://alexcican.com/post/guide-hosting-website-dropbox-github). The only problem I was left with was where I should host the blog. I decided to use Tumblr. Regretting the decision after only two months, the solution was right in front of me the entire time: **use Dropbox for the blog** as well. In this article I’ll show you how I did it.
<!--more-->
...
![scriptogram synchronize button](https://alexcican.com/images/blog/assets/scriptogram-publish-button.jpg)
...
![godaddy dns editor](https://alexcican.com/images/blog/assets/godaddy-dns-editor.jpg)

Based on the type of custom domain name you’ve added in the Scriptogr.am’s admin panel, we’ll make different changes to the DNS. For a sub-domain like `http://www.alexcican.com` or `http://blog.alexcican.com` you would simply create a `CNAME` record pointing to `scriptogr.am`. If you are using a top-level domain like `https://alexcican.com`, you must use an `A (Host)` record pointing to `50.116.1.192`. There’s no need to add a `CNAME` record for top-level domains. Wait up to 48-72 hours for the changes to take effect (usually within an hour or so).
